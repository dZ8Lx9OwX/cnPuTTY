...
static void create_controls(HWND hwnd, char *path)
{
    ...
    if (!path[0]) {
         ...
         */
        ctlposinit(&cp, hwnd, 3, 3, 243);   //original=235
        wc = &ctrls_base;
...
bool do_config(Conf *conf)
{
    ...
    dp.wintitle = dupprintf("cn%s 配置", appname);
    dp.errtitle = dupprintf("cn%s 错误", appname);
    dp.data = conf;
...
bool do_reconfig(HWND hwnd, Conf *conf, int protcfginfo)
{
    ...
    dp_add_tree(&dp, &ctrls_base);
    dp_add_tree(&dp, &ctrls_panel);
    dp.wintitle = dupprintf("cn%s 重新配置", appname);
    dp.errtitle = dupprintf("cn%s 错误", appname);
    dp.data = conf;
    dlg_auto_set_fixed_pitch_flag(&dp);
    dp.shortcuts['g'] = true;          /* the treeview: `Cate&gory' */
